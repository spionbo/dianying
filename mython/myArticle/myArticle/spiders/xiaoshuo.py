...
class XiaoshuoSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            yield self.request(url,self.parse,False)
    ...
    def parse(self, response):
        data = []
        generator = self.getItem(response)
        ...
        #     #进文章列表页及文章相关说明
        #     if(link):
        #         obj = yield self.request(link,self.getList,item)
        #         print(obj)
                # if(obj):
    ...
    def getList(self,response):
        ...
        item["dec"] = response.css("div.intro::text").extract_first()
        return item
        pass
